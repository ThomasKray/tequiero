...
allPaths.forEach(path => {
  ...
  // Mantener colores originales del SVG
...
setTimeout(() => {
  allPaths.forEach((path, i) => {
    ...
    path.style.transition =
      `stroke-dashoffset 1.3s cubic-bezier(.77,0,.18,1) ${i * 0.06}s,
       fill-opacity 0.5s ${1 + i * 0.06}s`;
...
const totalDuration = 1300 + (allPaths.length - 1) * 60 + 500;
...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Para mi persona favorita:\n\nNo sé si esto es un montón o es muy poco, sobre todo porque no te tengo acá para hacerte un regalo.\n\nNunca pensé que iba a hacer algo así; siempre se me hizo muy cursi y no soy bueno para estas cosas, pero con vos me dan ganas de intentarlo, aunque sea para sacarte una sonrisa.\n\nA pesar de los kilómetros de distancia que nos separan, ¿te gustaría pasar San Valentín conmigo?`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : `Te quiero, T.`;
...
// Cuenta regresiva hasta el 18 de febrero
function showCountdown() {
  const container = document.getElementById('countdown');

  // Año actual
  const now = new Date();
  let targetDate = new Date(`${now.getFullYear()}-02-18T00:00:00`);

  // Si ya pasó este 18 de febrero, usar el del próximo año
  if (now > targetDate) {
    targetDate = new Date(`${now.getFullYear() + 1}-02-18T00:00:00`);
  }
  ...
  function update() {
    ...
    if (diff <= 0) {
      container.innerHTML = `💖 ¡Ya nos estamos viendo! 💖`;
      return;
    }
    ...
    container.innerHTML =
  `<span style="color:#e60026;">Falta para vernos:</span><br>
